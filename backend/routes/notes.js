...
//Route3: Update a existing Note using PUT  "/api/notes/updatenote" login require
...
//Route4: Delete a existing Note using DELETE  "/api/notes/updatenote" login require
...
// Delete note func & give response
        note = await Note.findByIdAndDelete(req.params.id)
        res.json({ Successfully: "Your notes has been deleted Successfully", note: note })
        //Server error catch if upper code note work 
    } catch (error) {
        console.log(error.message)
        res.status(500).send("Internal Server Error occured")
    }
